...
exports.getPDFReport = async (req, res) => {
  try {
    ...
    if (workouts.length === 0) {
      doc.fontSize(12).text("No workouts found.\n");
    } else {
      workouts.forEach((w) => {
        w.exercises.forEach((ex) => {
          doc
            ...
            .text(`  Sets: ${ex.sets} | Reps: ${ex.reps} | Weight: ${ex.weight}`)
        ...
        n.meals.forEach((meal) => {
          meal.items.forEach((item) => {
            doc
              ...
              .text(
                `  Macros — Protein: ${item.macros.protein || 0}g, Carbs: ${item.macros.carbs || 0}g, Fat: ${item.macros.fat || 0}g`
              )
              .moveDown();
          });
        });
